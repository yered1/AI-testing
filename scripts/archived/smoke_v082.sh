...
ENG=$(curl -s -X POST "$API/v1/engagements" "${HDR[@]}" -d '{"name":"demo-082","tenant_id":"'"$TENANT"'","type":"web","scope":{"in_scope_domains":["example.com"]}}' | jq -r .id)

echo "== Auto-plan (v2)"
AUTO=$(curl -s -X POST "$API/v2/engagements/$ENG/plan/auto" "${HDR[@]}" -d '{"preferences":{"packs":[]}}')
...
curl -s "$API/v2/runs/$RUN/findings" "${HDR[@]}" | jq .
...
echo "Enrollment token issued (length):" ${#TOKEN}

echo "== Recent runs"
curl -s "$API/v2/runs/recent" "${HDR[@]}" | jq .
