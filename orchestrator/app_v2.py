...
def job_events(job_id: str, body: JobEventIn,
    ...
    # Flip job to running on 'started' events
    et = (body.type or "").lower()
    if any(k in et for k in ["job.started","job.running","adapter.started"]):
...
def job_complete(job_id: str, body: JobCompleteIn,
                 x_tenant_id: str = Header(alias="X-Tenant-Id"),
                 x_agent_id: str = Header(alias="X-Agent-Id"),
                 x_agent_key: str = Header(alias="X-Agent-Key"),
                 db: Session = Depends(get_db)):
    ...
    if status not in ("succeeded","failed","canceled","cancelled","skipped"):
        status = "succeeded"
    db.execute(text("UPDATE jobs SET status=:s WHERE id=:id"), {"s": status, "id": job_id})
    insert_event(db, x_tenant_id, job["run_id"], f"job.{status}", body.result or {})
    # Check run completion
    pending = db.execute(text("SELECT COUNT(*) FROM jobs WHERE run_id=:r AND status IN ('queued','leased','running')"),
                         {"r": job["run_id"]}).scalar()
    if int(pending or 0) == 0:
        failed = db.execute(text("SELECT COUNT(*) FROM jobs WHERE run_id=:r AND status IN ('failed','canceled','cancelled')"),
                            {"r": job["run_id"]}).scalar()
        run_status = "failed" if int(failed or 0) > 0 else "completed"
        db.execute(text("UPDATE runs SET status=:s WHERE id=:r"), {"s": run_status, "r": job["run_id"]})
        insert_event(db, x_tenant_id, job["run_id"], f"run.{run_status}", {"message": "All jobs finished"})
    db.commit()
    return {"ok": True}
