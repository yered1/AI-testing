...
def job_events(job_id: str,
    ...
    # Authenticate agent and ensure tenant isolation
    require_agent(db, x_tenant_id, x_agent_id, x_agent_key)
    job = db.execute(text("SELECT id, run_id, status FROM jobs WHERE id=:id"),
                     {"id": job_id}).mappings().first()
    if not job:
        raise HTTPException(status_code=404, detail="job not found")

    # Persist the job-scoped event
    db.execute(text("INSERT INTO job_events (id, tenant_id, job_id, type, payload) VALUES (:id,:t,:j,:ty,:pl)"),
               {"id": f"je_{uuid.uuid4().hex[:10]}", "t": x_tenant_id, "j": job_id, "ty": body.type, "pl": json.dumps(body.payload or {})})

    # If the adapter reports that it started, mark the job as running
    if body.type in ("job.started", "job.running", "adapter.started"):
        db.execute(text("UPDATE jobs SET status='running' WHERE id=:id AND status IN ('queued','leased')"),
                   {"id": job_id})

    # Emit a run-scoped event for UI streaming
    insert_event(db, x_tenant_id, job["run_id"], f"job.{body.type.split('.')[-1]}", body.payload or {})
    db.commit()
    return {"ok": True}
...
def runs_recent(limit: int = Query(50, ge=1, le=200), db: Session = Depends(get_db), principal: Principal = Depends(require_tenant)):
    ...
    return {"runs": [dict(x) for x in rows]}@app.post("/v2/jobs/{job_id}/complete")
def job_complete(job_id: str,
    ...
    # Auth
    require_agent(db, x_tenant_id, x_agent_id, x_agent_key)
    job = db.execute(text("SELECT id, run_id FROM jobs WHERE id=:id"),
                     {"id": job_id}).mappings().first()
    if not job:
        raise HTTPException(status_code=404, detail="job not found")

    # Update job terminal status
    db.execute(text("UPDATE jobs SET status=:s WHERE id=:id"),
               {"s": body.status, "id": job_id})

    # Run-scoped event
    insert_event(db, x_tenant_id, job["run_id"], f"job.{body.status}", body.result or {})

    # If all jobs have finished (no queued/leased/running), close the run; mark failed if any job failed
    remaining = db.execute(text("SELECT COUNT(1) FROM jobs WHERE run_id=:r AND status IN ('queued','leased','running')"),
                           {"r": job["run_id"]}).scalar()
    if int(remaining or 0) == 0:
        failed = db.execute(text("SELECT COUNT(1) FROM jobs WHERE run_id=:r AND status='failed'"),
                            {"r": job["run_id"]}).scalar()
        new_status = 'failed' if int(failed or 0) > 0 else 'completed'
        db.execute(text("UPDATE runs SET status=:s WHERE id=:r"), {"s": new_status, "r": job["run_id"]})
        insert_event(db, x_tenant_id, job["run_id"], f"run.{new_status}", {"message": "All jobs finished"})
